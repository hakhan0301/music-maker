...
def piano_roll_from_network(model, length_in_sec):
    ...
    for i in range(seq_count):
        model_input = model.predict([np.array(output[i])])
        output.append(model_input)
...
# model = model_maker.create_conv_model()
# print(model.summary())
# model_trainer.train_model(model)

# model_io.save_model("jazz_-1conv_cre_softm_8u_96seq_30epoch", model)

model = model_io.load_model_from_name("220151-jazz_-1conv_cre_softm_8u_96seq_30epoch")
make_song(model, 30)
...
# 152120-jazz_mse_tanh_48_30epoch_30seq.model
